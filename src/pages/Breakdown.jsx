...
/**
 * Animated category bar chart — pure CSS + framer-motion. Exposes a
 * progressbar role with ARIA value attributes for assistive tech.
 */
function CategoryBar({ label, kg, totalKg, color, delay = 0 }) {
  ...
  return (
    ...
    >
      ...
      <div
        className="h-3 rounded-full bg-earth-100 dark:bg-earth-800 overflow-hidden"
        role="progressbar"
        aria-valuenow={Math.round(pct)}
        aria-valuemin={0}
        aria-valuemax={100}
        aria-label={`${label}: ${pct.toFixed(0)}% of footprint`}
      >
        <motion.div
...
/** Horizontal comparison bar — user (green/amber) vs a reference value. */
...
/**
 * Breakdown page — detailed annual carbon footprint by category, with global
 * per-capita comparisons and the Paris-aligned target. Requires a completed
 * survey; shows the NotOnboarded empty state otherwise.
 */
  ...
  if (!hasCompleted || !footprint) {
    return (
      <NotOnboarded
        ...
        desc="Complete the quick onboarding survey to get your personalised carbon footprint breakdown."
      />
    );
  }
        ...
        <Reveal>
          <div className="card p-6 sm:p-8 mb-8">
            <h2 className="text-xl font-bold text-earth-950 dark:text-white mb-6 flex items-center gap-2">
              <Target className="h-5 w-5 text-leaf-500" aria-hidden="true" />
            ...
            <div className="card border-leaf-200 dark:border-leaf-900 bg-leaf-50/50 dark:bg-leaf-950/20 p-6 mb-8">
              <div className="flex items-start gap-4">
                <div className="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-leaf-500/15">
                  <TrendingDown className="h-6 w-6 text-leaf-600" aria-hidden="true" />
                </div>
                <div>
                  ...
                  >
                    See personalised actions <ArrowRight className="h-3.5 w-3.5" aria-hidden="true" />
                  </Link>
                </div>
              </div>
            </div>
